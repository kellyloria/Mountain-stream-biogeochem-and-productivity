...
covariat_dat_clean <- covariat_dat %>%
  dplyr::filter(!if_all(c(bulk.density, AFDM_mgg, AFDM_mgcm2, Chla_ugL_Q, Pheo_ugL_Q), is.na))

# write.csv(covariat_dat_clean, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_benthic_biomass_OM.csv")
...
covariat_WQ <- WQ_dat%>%
  ...
  dplyr::filter(site=="BWL"| site=="BWU"| site=="GBL"| site=="GBU")

# write.csv(covariat_WQ, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_nutrients.csv")


covariat_datq <- covariat_dat%>%
  left_join(covariat_WQ,  by=c("date", "site"))
...
# write.csv(SPC_datD, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_SPC.csv")
...
# write.csv(Morph_dat_day, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_stream_morphology.csv")
...
# write.csv(BWL_dat, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_BWL_miniDOT.csv")
...
# write.csv(BWU_dat, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_BWU_miniDOT.csv")
...
# write.csv(GBL_dat, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_GBL_miniDOT.csv")
...
# write.csv(GBU_dat, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_GBU_miniDOT.csv")
...
# write.csv(metab_dat, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_metab.csv")
...
# write.csv(nitrogen_data, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_nutrients.csv")
...
# write.csv(covariat_ndemand, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_N_demand.csv")
...
#install.packages("ggpmisc")  # if not already installed
library(ggpmisc)
library(dplyr)
...
covariat_datqI <- covariat_datqI %>%
  mutate(date = as.Date(date),
         day_num = as.numeric(date))  # numeric for modeling (slope per day)

# Compute slope, r, p-value per site
stats_df <- covariat_datqI %>%
  #filter(date > as.Date("2022-08-30")) %>%
  group_by(site) %>%
  summarise(
    slope = coef(lm(AFDM_mgg ~ day_num))[2],
    r     = cor(day_num, AFDM_mgg, use = "complete.obs"),
    p     = summary(lm(AFDM_mgg ~ day_num))$coefficients["day_num", "Pr(>|t|)"],
    .groups = "drop"
  ) %>%
  mutate(
    label = paste0(
      "slope = ", signif(slope, 2), " (mg^g)\n",
      "r = ", signif(r, 2), "\n",
      "p = ", format.pval(p, digits = 2, eps = 0.001)
    )
  )

TS_plot_w <- ggplot(covariat_datqI, aes(x = date, y = AFDM_mgg, color = site, shape = site)) +
  geom_point(size = 2, alpha = 0.5, position = position_dodge(width = 0.3)) +
  geom_point(size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE) +
  scale_color_manual(values = siteC_colors) +
  scale_shape_manual(values = c(15, 0, 17, 2)) +
  scale_x_date(limits = as.Date(c("2022-08-01", "2023-10-01"))) +
  labs(
    x = "Date",
    y = expression("OM (" * mg ~ g^{-1} ~ "sediment)")
  ) +
  facet_grid(site ~ .) +
  geom_text(
    data = stats_df,
    aes(x = as.Date("2022-10-05"), y = Inf, label = label),
    inherit.aes = FALSE,
    hjust = 0, vjust = 1.1,
    size = 3
  ) +
...
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/Sup_figure_OM_TS.png", plot = TS_plot_w, width = 6, height = 8, units = "in")
...
stats_df <- covariat_datqI %>%
  ...
  group_by(site) %>%
  summarise(
    slope = coef(lm(AFDM_mgcm2 ~ day_num))[2],
    r     = cor(day_num, AFDM_mgcm2, use = "complete.obs"),
    p     = summary(lm(AFDM_mgcm2 ~ day_num))$coefficients["day_num", "Pr(>|t|)"],
    .groups = "drop"
  ) %>%
  mutate(
    label = paste0(
      "slope = ", signif(slope, 2), " (mg^cm^2)\n",
      "r = ", signif(r, 2), "\n",
      "p = ", format.pval(p, digits = 2, eps = 0.001)
    )
  )

TS_plot_w <- ggplot(covariat_datqI,#%>%filter(date > as.Date("2022-09-30")), 
                    aes(x = date, y = AFDM_mgcm2, color = site, shape = site)) +
  geom_point(size = 2, alpha = 0.5, position = position_dodge(width = 0.3)) +
  geom_point(size = 2, alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE) +
  scale_color_manual(values = siteC_colors) +
  scale_shape_manual(values = c(15, 0, 17, 2)) +
  scale_x_date(limits = as.Date(c("2022-08-30", "2023-08-30"))) +
  labs(
    x = "Date",
    y = expression("AFDM (" * mg ~ cm^{-2} ~ ")")
  ) +
  facet_grid(site ~ .) +
  geom_text(
    data = stats_df,
    aes(x = as.Date("2022-09-05"), y = Inf, label = label),
    inherit.aes = FALSE,
    hjust = 0, vjust = 1.1,
    size = 3
  ) +
...
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/Sup_figure_ADFM_TS.png", plot = TS_plot_w, width = 6, height = 8, units = "in")
...
ndyn_save <- ndyn_save %>%
  mutate(NO3_supply_new1 = coalesce(NO3_supply_new, NO3_supply),
         NH4_supply_new1 = coalesce(NH4_supply_new, NH4_supply)
         )

ndyn_save <- ndyn_save %>%
  group_by(site, date) %>%
  summarise(across(everything(), mean, na.rm = TRUE), .groups = "drop") %>%
  dplyr::select(date, site, reachL, NO3_supply_new1, NH4_supply_new1, PO4_supply, Ndemand)




## write.csv(ndyn_save, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_modeled_N_dynamics.csv")



ndyn_save <- ndyn_save %>%
  filter(!is.na(NO3_supply_new1) & !is.na(NH4_supply_new1) & !is.na(reachL))

## write.csv(ndyn_save, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_modeled_N_dynamics.csv")


N_demand_data <- read.csv("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_N_demand.csv")

str(N_demand_data)

N_supply <- read.csv("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_modeled_N_dynamics.csv")


### temp revision section 


N_dem_bw <- N_supply %>%
  filter(site %in% c("GBU", "GBL")) %>%
  mutate(wy_lab = case_when(
    date <= as.Date("2022-09-30") ~ "dry",
    date >  as.Date("2022-09-30") |   date <  as.Date("2023-09-30")~ "wet"
  ))


N_dem_bw <- N_dem_bw %>%
  mutate(wy_lab = factor(wy_lab, levels = c("dry", "wet")))

anova_mod <- aov(Ndemand ~ wy_lab, data = N_dem_bw)
summary(anova_mod)

pct_increase <- N_dem_bw %>%
  summarise(
    dry_N = mean(Ndemand[wy_lab == "dry"], na.rm = TRUE),
    wet_N = mean(Ndemand[wy_lab == "wet"], na.rm = TRUE),
  ) %>%
  mutate(
    pct_inc_N = (wet_N - dry_N) / dry_N * 100
  )
...
Nydn_save <- full_join(N_demand_data, N_supply, by = c("site", "date"))

## write.csv(Nydn_save, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_modeled_N_dynamics1.csv")
...
N_supply_bwl_dry <- N_supply %>%
  filter(site=="BWL" & date < as.Date("2022-09-30")) 
summary(N_supply_bwl_dry)

N_supply_bwl_wet <- N_supply %>%
  filter(site=="BWL" & date > as.Date("2022-09-30") |  date < as.Date("2023-09-30")) 
summary(N_supply_bwl_wet)


N_supply_bwu_dry <- N_supply %>%
  filter(site=="BWU" & date < as.Date("2022-09-30"))
summary(N_supply_bwu_dry)

N_supply_bwu_wet <- N_supply %>%
  filter(site=="BWU" & date > as.Date("2022-09-30") |  date < as.Date("2023-09-30"))
summary(N_supply_bwu_wet)

N_supply_gbl_dry <- N_supply %>%
  filter(site=="GBL" & date < as.Date("2022-09-30"))
summary(N_supply_gbl_dry)

N_supply_gbl_wet <- N_supply %>%
  filter(site=="GBL" & date > as.Date("2022-09-30"))
summary(N_supply_gbl_wet)

N_supply_gbu_dry <- N_supply %>%
  filter(site=="GBU" & date < as.Date("2022-09-30"))
summary(N_supply_gbu_dry)

N_supply_gbu_wet <- N_supply %>%
  filter(site=="GBU"  & date > as.Date("2022-09-30"))
summary(N_supply_gbu_wet)

### quick anova 

N_supply_GB <- N_supply %>%
  filter(site %in% c("GBL", "GBU")) %>%
  mutate(wy_lab = case_when(
    date <= as.Date("2022-09-30") ~ "dry",
    date >  as.Date("2022-09-30") |   date <  as.Date("2023-09-30")~ "wet"
  ))
...
N_supply_BW <- N_supply %>%
  filter(site %in% c("BWL", "BWU")) %>%
  mutate(wy_lab = case_when(
    date <= as.Date("2022-09-30") ~ "dry",
    date >  as.Date("2022-09-30") |   date <  as.Date("2023-09-30")~ "wet"
  ))

N_supply_bw <- N_supply_bw %>%
  mutate(wy_lab = factor(wy_lab, levels = c("dry", "wet")))

anova_mod <- aov(NO3_supply ~ wy_lab, data = N_supply_bw)
summary(anova_mod)

anova_mod <- lm(NH4_supply ~ wy_lab, data = N_supply_bw)
Anova(anova_mod, type = 2)

anova_mod <- lm(NH4_supply ~ wy_lab, data = N_supply_bw)
summary(anova_mod)

pct_increase <- N_supply_bw %>%
  summarise(
    dry_NO3 = mean(NO3_supply[wy_lab == "dry"], na.rm = TRUE),
    wet_NO3 = mean(NO3_supply[wy_lab == "wet"], na.rm = TRUE),
    dry_NH4 = mean(NH4_supply[wy_lab == "dry"], na.rm = TRUE),
    wet_NH4 = mean(NH4_supply[wy_lab == "wet"], na.rm = TRUE)
  ) %>%
  mutate(
    pct_inc_NO3 = (wet_NO3 - dry_NO3) / dry_NO3 * 100,
    pct_inc_NH4 = (wet_NH4 - dry_NH4) / dry_NH4 * 100
  )
...
###
# Add combined N supply
N_supply_BW$N_supply_total <- N_supply_BW$NO3_supply + N_supply_BW$NH4_supply
N_supply_GB$N_supply_total <- N_supply_GB$NO3_supply + N_supply_GB$NH4_supply

# Mean combined supply
mean_BW <- mean(N_supply_BW$N_supply_total, na.rm = TRUE)
mean_GB <- mean(N_supply_GB$N_supply_total, na.rm = TRUE)

# Difference (BW - GB)
difference <-  round(mean_GB - mean_BW, 2)
...
Baseflowplots <- covariat_datq %>%
  ...
  left_join(steepest_slopes %>% dplyr::select(site, water_year, mean_q_yday, ave_q), by = c("site", "water_year","yday"= "mean_q_yday")) %>%
    left_join(overall_pf %>%dplyr::select(site, water_year, max_pf, max_pf_yday), by = c("site", "water_year", "yday"= "max_pf_yday")) %>%filter(site=="BWL" | site=="GBL")
...
# write.csv(uptake, "/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/EDI_datasets/TSMQ_nitrogen_uptake.csv")
...
BM_cor_plt <- ggplot(covariat_datq, aes(x =AFDM_ugcm2, y = GPP_mean)) + 
  ...
  xlab(expression(log(Biomass)~(µg~cm^-2))) +
  scale_color_manual(values = catch_colors)
...
BM_cor_plt1 <- ggplot(covariat_datq, aes(x = (Chla_ugL_Q), y = (GPP_mean))) + 
  ...
  ylab(expression(log(GPP)~(g~O[2]~m^-2~d^-1))) +
  xlab(expression(log(Chla)~(µg~L^-1))) +
  scale_color_manual(values = catch_colors)
  ...
  theme_classic() + 
      ...
      scale_y_continuous(trans = "log", 
                         breaks = scales::log_breaks(base = 10), 
                           labels = scales::label_number()) + 
  xlab(expression(log(Biomass)~(µg~cm^-2))) +
  ylab(expression(log(Chla)~(µg~L^-1))) +
  scale_color_manual(values = catch_colors)
...
Stoich_df <- ndyn %>%
  mutate(
    jday= yday(date),
    NO3_uM_pw = (NO3_mgL_dl_pw * 1000) / 62.0049,  # Convert NO3 to µmol/L
    NH4_uM_pw = (NH4_mgL_dl_pw * 1000) / 18.0385,  # Convert NH4 to µmol/L
    PO4_uM_pw = PO4_ugL_dl_pw / 30.9738,  # Convert PO4 to µmol/L
    Tot_N_uM_pw = NO3_uM_pw + NH4_uM_pw,  # Total nitrogen in µmol/L
    N_P_ratio_pw = if_else(PO4_uM_pw > 0, Tot_N_uM_pw / PO4_uM_pw, NA_real_),  # Avoid division by zero
    
    NO3_uM_sw = (NO3_mgL_i * 1000) / 62.0049,  # Convert NO3 to µmol/L
    NH4_uM_sw = (NH4_mgL_i * 1000) / 18.0385,  # Convert NH4 to µmol/L
    PO4_uM_sw = PO4_ugL_dl_sw.y / 30.9738,  # Convert PO4 to µmol/L
    Tot_N_uM_sw = NO3_uM_sw + NH4_uM_sw,  # Total nitrogen in µmol/L
...
plot_BW_OM <- ggplot(
  covariat_dat_bp %>% 
    filter(site == "BWL", water_year %in% 2022:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = AFDM_mgg)) +
...
plot_BWU_OM <- ggplot(
  covariat_dat_bp %>% 
    filter(site == "BWU", water_year %in% 2022:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = AFDM_mgg)) +
 ...
 # scale_fill_manual(values = c("2021" = "#440154", "2022" = "#3B528B", "2023" = "#4bcc94")) +
  ylab(expression(OM~(mg~g^-1))) + 
  labs(x = "BWU Lower", fill = "Water Year", color = "Catchment", shape = "Catchment") +
  theme_minimal()
...
anova_n03_sup_bw <- aov(NO3_supply_new ~ as.factor(water_year), data = uptake_dyn2%>%filter(site == "BWL", water_year %in% 2021:2023))
...
anova_n03_sup_bw <- aov(NO3_supply_new ~ as.factor(water_year), data = uptake_dyn2%>%filter(site == "BWU", water_year %in% 2021:2023))
summary(anova_n03_sup_bw)
...
anova_n03_sup_gb <- aov(NO3_supply_new ~ as.factor(water_year), data = uptake_dyn2%>%filter(site == "GBL", water_year %in% 2021:2023))
...
### anova 
anova_n03_bw<- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "BWL", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n03_bw)

# Convert to compact letter display
tukey_letters <- multcompLetters4(anova_n03_bw, tukey_result)
letters_df <- data.frame(
  water_year = names(tukey_letters[[1]]$Letters),
  letters = tukey_letters[[1]]$Letters
)

letters_df$water_year <- factor(letters_df$water_year, levels = desired_years)


plot_Nsupply_BWL <- ggplot(
  uptake_dyn2 %>% 
    filter(site == "BWL", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NO3_supply_new)) +
  
  geom_boxplot(alpha = 0.5, outlier.shape = NA, aes(fill=catch, color=catch)) +
  geom_jitter(aes(color = factor(catch)),
              size = 1.5, alpha = 0.95,
              width = 0.2, height = 0, shape = 15) +
  ...
  #scale_fill_manual(values = c("2021" = "#440154", "2022" = "#3B528B", "2023" = "#4bcc94")) +
  ylab(expression(NO[3]~"supply")) +
    xlab(NULL) + 
  labs(fill = "Water Year", color = "Catchment", shape = "Catchment") +
  ylim(0, 2.0) +
  theme_classic()

plot_Nsupply_BWL <- plot_Nsupply_BWL +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 1.95, label = letters),  
    inherit.aes = FALSE,
    size = 3
  )
...
### anova 
anova_n <- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "BWU", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)

# Convert to compact letter display
tukey_letters <- multcompLetters4(anova_n, tukey_result)
letters_df <- data.frame(
  water_year = names(tukey_letters[[1]]$Letters),
  letters = tukey_letters[[1]]$Letters
)
...
plot_Nsupply_BWU <- ggplot(
  uptake_dyn2 %>% 
    filter(site == "BWU", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NO3_supply_new)) +
  
  geom_boxplot(alpha = 0.5, outlier.shape = NA, aes(fill=catch, color=catch)) +
  geom_jitter(aes(color = factor(catch)),
              size = 1.5, alpha = 0.95,
              width = 0.2, height = 0, shape = 0) +
 ...
 ylab(NULL) +  
        xlab(NULL) +
  labs(fill = "Water Year", color = "Catchment", shape = "Catchment") +ylim(0, 2.0) +
  theme_classic()


plot_Nsupply_BWU <- plot_Nsupply_BWU +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 1.95, label = letters),  
    inherit.aes = FALSE,
...
### anova 
anova_n <- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "GBL", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
plot_Nsupply_GBL <- ggplot(
  uptake_dyn2 %>% 
    filter(site == "GBL", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NO3_supply_new)) +
  
  geom_boxplot(alpha = 0.5, outlier.shape = NA, aes(fill=catch, color=catch)) +
  geom_jitter(aes(color = factor(catch)),
              size = 1.5, alpha = 0.95,
              width = 0.2, height = 0, shape = 17) +
...
plot_Nsupply_GBL <- plot_Nsupply_GBL +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 1.95, label = letters),  
    inherit.aes = FALSE,
...
### anova 
anova_n <- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "GBU", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
plot_Nsupply_GBU <- ggplot(
  uptake_dyn2 %>% 
    filter(site == "GBU", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NO3_supply_new)) +
  
  geom_boxplot(alpha = 0.5, outlier.shape = NA, aes(fill=catch, color=catch)) +
  geom_jitter(aes(color = factor(catch)),
              size = 1.5, alpha = 0.95,
              width = 0.2, height = 0, shape = 2) +
...
plot_Nsupply_GBU <- plot_Nsupply_GBU +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 1.95, label = letters),  
    inherit.aes = FALSE,
...
### anova 
anova_n <- aov(NH4_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "BWL", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
summary(anova_n)
...
plot_NH4supply_BWL <- ggplot(
  uptake_dyn2 %>% 
    filter(site == "BWL", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NH4_supply_new)) +
...
plot_NH4supply_BWL <- plot_NH4supply_BWL +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.62, label = letters),  
    inherit.aes = FALSE,
    size = 3
  )
...
plot_NH4supply_BWU <- plot_NH4supply_BWU +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.62, label = letters),  
    inherit.aes = FALSE,
...
plot_NH4supply_GBL <- plot_NH4supply_GBL +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.62, label = letters),  
    inherit.aes = FALSE,
...
plot_NH4supply_GBU <- plot_NH4supply_GBU +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.62, label = letters),  
    inherit.aes = FALSE,
...
### anova 
anova_n <- aov(Ndemand ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "BWL", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
plotNdemandBWL <- ggplot(
  uptake_dyn2 %>% 
    filter(site == "BWL", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = Ndemand)) +
...
# Run ANOVA and TukeyHSD
anova_n_sup <- aov(Ndemand ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "BWU", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n_sup)

# Convert to compact letter display
tukey_letters <- multcompLetters4(anova_n_sup, tukey_result)
letters_df <- data.frame(
  water_year = names(tukey_letters[[1]]$Letters),
  letters = tukey_letters[[1]]$Letters
)
...
# Convert to compact letter display
tukey_letters <- multcompLetters4(anova_n_sup_gb, tukey_result)
letters_df <- data.frame(
  water_year = names(tukey_letters[[1]]$Letters),
  letters = tukey_letters[[1]]$Letters
)
...
anova_n <- aov(c(frac_NH4_uptake) ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "BWL", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
uptake_dyn3 <- uptake_dyn2%>%
  mutate(frac_N_uptake= sum(frac_NO3_uptake, frac_NH4_uptake))

plotfrac_N_uptakeBWL <- ggplot(
  uptake_dyn2 %>% 
    filter(site == "BWL", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = frac_NO3_uptake)) +
  ...
  scale_fill_manual(values = catch_colors) +
  #scale_fill_manual(values = c("2021" = "#440154", "2022" = "#3B528B", "2023" = "#4bcc94")) +
  ylab(expression(N~"uptake")) + 
    xlab("BW Lower") +
  labs(fill = "Water Year", color = "Catchment", shape = "Catchment") +
  ylim(0, 1) +
  theme_classic()

plotfrac_N_uptakeBWL <- plotfrac_N_uptakeBWL +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.97, label = letters),  
    inherit.aes = FALSE,
    size = 3
  )


anova_n <- aov(frac_NO3_uptake ~ as.factor(water_year), 
                        data = uptake_dyn2 %>% filter(site == "BWU", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
plotfrac_N_uptakeBWU <- plotfrac_N_uptakeBWU +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.97, label = letters),  
    inherit.aes = FALSE,
...
plotfrac_N_uptakeGBL <- ggplot(
  ...
  scale_fill_manual(values = catch_colors) +
  #scale_fill_manual(values = c("2021" = "#440154", "2022" = "#3B528B", "2023" = "#4bcc94")) +
  ylab(NULL) + xlab("GB Lower") +
  #ylab(expression(N~"uptake")) + 
  labs(fill = "Water Year", color = "Catchment", shape = "Catchment") +
  ylim(0,  1) +
  theme_classic()

plotfrac_N_uptakeGBL <- plotfrac_N_uptakeGBL +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.97, label = letters),  
    inherit.aes = FALSE,
...
plotfrac_N_uptakeGBU <- ggplot(
  ...
  scale_fill_manual(values = catch_colors) +
  #scale_fill_manual(values = c("2021" = "#440154", "2022" = "#3B528B", "2023" = "#4bcc94")) +
  ylab(NULL) + xlab("GB Upper") +
  labs(fill = "Water Year", color = "Catchment", shape = "Catchment") +
  ylim(0,  1) +
  theme_classic()


plotfrac_N_uptakeGBU <- plotfrac_N_uptakeGBU +
  geom_text(
    data = letters_df,
    aes(x = water_year, y = 0.97, label = letters),  
    inherit.aes = FALSE,
...
Ndyn_annual_grid <- ggarrange(
  ...
  plotNdemandGBL,
  plotNdemandGBU,
  plotfrac_N_uptakeBWL,
  plotfrac_N_uptakeBWU,
  plotfrac_N_uptakeGBL,
  plotfrac_N_uptakeGBU,
  align = "v",
    ncol = 4, nrow = 4,
    common.legend = TRUE, 
   legend = "right")
...
date_datf_plt_sum <- covariat_datq %>%
    mutate(year=year(date))%>%
  filter(yday>1 & yday<285)%>%
  filter(site=="BWL" & year ==2024) %>%
  group_by(year) %>%
  summarise(T_q = max(Q_m, na.rm = TRUE),
            T_wtr = max(wtr_m, na.rm = TRUE),
            T_spc = max(SPC_m, na.rm = TRUE),
            T_om = max(AFDM_mgg, na.rm = TRUE),
            T_biomass = max(AFDM_ugcm2, na.rm = TRUE),
            T_chla = max(Chla_ugL_Q, na.rm = TRUE))




anova_n <- aov(NO3_processedi ~ as.factor(water_year), 
                        data = reach_rent %>% filter(catch == "BW", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)

summary(anova_n)


anova_n <- aov(NO3_processed ~ as.factor(water_year), 
                        data = reach_rent %>% filter(catch == "GB", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
