...
covariat_nsupply <- covariat_datq %>%
  dplyr::select(site, catch, date, water_year,
                reach_length,
                v_m,w_m, Q_m, substrate, K600_daily_mean,
                NO3_mgL_dl,NH4_mgL_dl, PO4_ugL_dl)%>%
  ...
  mutate(
    K600_daily_mean = case_when(
      is.na(K600_daily_mean) & site == "BWL" ~ 22, # mean measured estimates o k600
  ...
  mutate(
    reach_length = case_when(
      is.na(reach_length) & site == "BWL" ~ 125,
      is.na(reach_length) & site == "BWU" ~ 100,
      is.na(reach_length) & site == "GBU" ~ 90,
      is.na(reach_length) & site == "GBL" ~ 90,
      TRUE ~ reach_length
    )
  ) %>%
  mutate(
  ...
  NO3_supply = c(((86400*Q_Ls*(NO3_mgL_dl/1000))/(w_mi*reachL))),
   # Calculate nh3 supply 
  NH4_supply = c(((86400*Q_Ls*(NH4_mgL_dl/1000))/(w_mi*reachL))), ## unit should be g N m-2 d-1
  PO4_supply = c(((86400*Q_Ls*(PO4_ugL_dl/1e-6))/(w_mi*reachL))) ## unit should be g N m-2 d-1
   )  %>%
...
#### Looking at the upper and lower reach differences:



#### Consider the units 

 - Areal max uptake -  Ut - ug / m2 min
...
uptake_df <- covariat_NS_ND %>%
    full_join(uptake, by=c ("site", "date"))
...
### plots for DOY 
color by year (2022 or 2023)
Looks like uptake is generally still higher in 2023
...
#### Alright so there could be a slight mismatch 
