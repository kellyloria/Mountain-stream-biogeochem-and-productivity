...
```{r, echo=FALSE, fig.width=9, fig.height=4, fig.align='center', inlcude=T}
...
plot_SWE <- snowflow_dat %>%
  ggplot(aes(y = accum_SWE/1000, x = date, color = site)) +
  geom_line(linewidth=1) + theme_classic() +
    scale_color_manual(values=alpha(c(site_colors), 0.75)) +
...
```{r, warning=F, echo=F,include=T, fig.width=7, fig.height=6}
...
snowflow_dat_sum <- snowflow_dat%>%
  ...
  summarise(Total_SWE = max(accum_SWE, na.rm=T),
            Water_yeild = max(accum_Qm3D, na.rm = T))
...
snowflow_HA <- snowflow_dat_sum%>%
  ...
  summarise(mean_SWE_acc = mean(Total_SWE, na.rm=T),
            mean_Water_yeild = mean(Water_yeild, na.rm = T))


SWP_acc_plots <- ggplot(snowflow_dat_sum, aes(x = WaterYear, y = Total_SWE/1000, color = site)) +
...
WYeild_acc_plots <- ggplot(snowflow_dat_sum, aes(x = WaterYear, y = Water_yeild, color = site)) +
...
WYeild_acc_plotss <- ggplot(snowflow_dat_sum%>%filter(WaterYear>2020), aes(x = WaterYear, y = Water_yeild, color = site)) +
  geom_point(size = 3, alpha = 0.95, position = position_dodge(width = 0.1), shape=17) +
  geom_line(position = position_dodge(width = 0.1))+
    geom_hline(yintercept = 1052085.5, col = "#054fb9", lty = 2) + 
  geom_hline(yintercept = 149169.5, col = "#DD6E42", lty = 2) + 
...
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/Hydro_pannel.png", plot = grid, width = 2.25, height = 4.5, units = "in")
