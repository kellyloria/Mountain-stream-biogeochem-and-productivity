...
lapply(c("dplyr","ggplot2","cowplot",
...
bg_nuts <- readRDS("/Users/kellyloria/Documents/LittoralMetabModeling/RawData/WaterChem/NS_chem_dat_nh4_24.rds") %>%
 dplyr::filter(location == "stream") %>%
  dplyr::mutate(
    NO3_mgL_dl = dplyr::if_else(NO3_mgL_dl < 0.0015, 0.0015, NO3_mgL_dl),
    NH4_mgL_dl = dplyr::if_else(NH4_mgL_dl < 0.001, 0.001, NH4_mgL_dl),
    PO4_ugL_dl = dplyr::if_else(PO4_ugL_dl < 0.201, 0.201, PO4_ugL_dl),
    DOC_mgL_dl = dplyr::if_else(DOC_mgL_dl < 0.25, 0.125, DOC_mgL_dl)
  ) %>%
  dplyr::group_by(site, shore, date, depth, location, substrate) %>%
...
bg_nuts_wide <- bg_nuts %>%
  ...
  dplyr::summarise(across(c(NO3_mgL_dl, NH4_mgL_dl, PO4_ugL_dl, DOC_mgL_dl, pH_infill), mean, na.rm = TRUE), .groups = "drop") %>% 
...
covariat_datqI <- covariat_datq %>%
  ...
  dplyr::mutate(
    w_m_apr = if_else(
      is.na(w_m),
      predict(lm(w_m ~ depth_m, data = dplyr::cur_data(), na.action = na.omit),
              newdata = dplyr::cur_data()),
      w_m
    ),
    v_m_apr = if_else(
      is.na(v_m),
      predict(lm(v_m ~ Q_m, data = dplyr::cur_data(), na.action = na.omit),
              newdata = dplyr::cur_data()),
      v_m
    )
  ) %>%
...
overall_bf<- covariat_datq %>%
   dplyr::filter(yday >= 170, yday <= 274) %>%
  group_by(site) %>%
 dplyr::summarise(
...
steepest_slopes <- overall_bf_df %>%
   dplyr::filter(yday >= 153, yday <= 274) %>%
...
Baseflowplots <- covariat_datq %>%
  ...
  left_join(steepest_slopes %>% dplyr::select(site, water_year, mean_q_yday, ave_q), by = c("site", "water_year","yday"= "mean_q_yday")) %>%
    left_join(overall_pf %>%dplyr::select(site, water_year, max_pf, max_pf_yday), by = c("site", "water_year", "yday"= "max_pf_yday")) %>%dplyr::filter(site=="BWL" | site=="GBL")
...
Baseflowplot <- ggplot(Baseflowplot1 %>% 
  ...
  geom_vline(data = Baseflowplot1 %>%dplyr::filter(water_year < 2025),
...
BF_sum <- Baseflowplot1%>%
 dplyr::filter(!is.na(Baseflowplot1$ave_q))
...
Baseflowplot <- ggplot(Baseflowplot1 %>% 
  ...
  geom_text(data = Baseflowplot1 %>% 
             dplyr::filter(site == "BWL" & water_year < 2025), 
  ...
  geom_text(data = Baseflowplot1 %>% 
             dplyr::filter(site == "GBL" & water_year < 2025), 
...
dplyr::filter(water_year<2025)%>%
 dplyr::filter(!is.na(GPP_mean) & !is.na(ER_mean))%>%
...
#  geom_contour(data = density_df, aes(x = x, y = y, z = z), color = "black", bins = 10) +
  geom_point(data = GBL_DF_clean%>%dplyr::filter(water_year<2025), aes(x = GPP_mean, y = ER_mean, color = as.factor(water_year)), alpha = 0.5) +
  scale_x_continuous(position = "top") +
      geom_contour(data = density_df, aes(x = x, y = y, z = z), color = "#f2e54b", bins = 25, size=0.25) +
  geom_abline(slope = -1, intercept = 0, color = "black") +  # 1:1 line
    #xlab(expression(GPP~(g~O[2]~m^-2~d^-1))) +
...
templt <- ggplot(covariat_datq, aes(x = date)) +
  ...
  geom_point(data =dplyr::filter(plot_data_DO, has_do), aes(y = 0.095), alpha=0.3, color = "black", size = 1) + # DO presence tick marks
  geom_point(data =dplyr::filter(plot_data_GPP, has_GPP), aes(y = 0.090), alpha= 0.3, color = "#579467", size = 1) + # GPP presence tick marks
    geom_point(data =dplyr::filter(plot_data_GPP, has_GPP), aes(y = 0.085), alpha= 0.02, color = "#fffcfd", size = 1) + # GPP presence tick marks
...
N_ratio_plots <- ggplot(covariat_NS_ND%>%dplyr::filter(water_year<2025), aes(x = (NO3_supply), y = (Ndemand), color = as.factor(water_year), shape=site)) +
...
NO3_supp_plots <- ggplot(covariat_NS_ND%>%dplyr::filter(water_year<2025), aes(y = (NO3_supply_new), x = jday, color = as.factor(water_year), shape=site)) +
...
N_demand_plot1 <- ggplot(covariat_NS_ND%>%dplyr::filter(water_year<2025), aes(y = (Ndemand), x = jday, color = as.factor(water_year), shape = site)) +
...
n_df_temp <- uptake_dyn %>%
  group_by(site) %>%
 dplyr::summarise(U_NO3_calc= c(mean(U_NO3_calc, na.rm=T)*1000),
...
hist1<- ggplot(uptake_dyn%>%dplyr::filter(water_year<2024), aes(x = log(S_D_NO3+1))) +
...
hist2<- ggplot(uptake_dyn%>%dplyr::filter(water_year<2024), aes(x = log(S_D_NH4+1))) +
 # geom_histogram(aes(fill = site,  color = site),  position = "dodge", alpha = 0.7, bins = 5) +  scale_color_manual(values = site_colors) + 
  geom_density(alpha = 0.7, aes(fill = site,  color = site)) +
  scale_color_manual(values = site_colors)  +
  scale_fill_manual(values = site_colors) + theme_minimal() +
...
uptake_dyn_sum <- uptake_dyn %>%
 dplyr::filter(!is.na(Ndemand) & !is.na(NO3_supply_new) & !is.na(NH4_supply_new))
...
lm_no3_d_gb <- lmer(log(U_NO3_calc+1) ~ scale(Ndemand) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="GB"))
summary(lm_no3_d_gb)
r.squaredGLMM(lm_no3_d_gb)

lm_no3_d_bw <- lmer(log(U_NO3_calc+1) ~ scale(Ndemand) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="BW"))
summary(lm_no3_d_bw)
r.squaredGLMM(lm_no3_d_bw)

lm_nh4_d_gb <- lmer(log(U_NH4_calc+1) ~ scale(Ndemand) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="GB"))
summary(lm_nh4_d_gb)
r.squaredGLMM(lm_nh4_d_gb)

lm_nh4_d_bw <- lmer(log(U_NH4_calc+1) ~ scale(Ndemand) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="BW"))
summary(lm_nh4_d_bw)
r.squaredGLMM(lm_nh4_d_bw)

lm_no3_s_gb <- lmer(log(U_NO3_calc+1) ~ scale(NO3_supply_new) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="GB"))
summary(lm_no3_s_gb)
r.squaredGLMM(lm_no3_s_gb)


lm_no3_s_bw <- lmer(log(U_NO3_calc+1) ~ scale(NO3_supply_new) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="BW"))
summary(lm_no3_s_bw)
r.squaredGLMM(lm_no3_s_bw)


lm_nh4_s_gb <- lmer(log(U_NH4_calc+1) ~ scale(NH4_supply_new) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="GB"))
summary(lm_nh4_s_gb)
r.squaredGLMM(lm_nh4_s_gb)


lm_nh4_s_bw <- lmer(log(U_NH4_calc+1) ~ scale(NH4_supply_new) + (1|water_year), data= uptake_dyn%>%dplyr::filter(catch=="BW"))
summary(lm_nh4_s_bw)
...
high_Ndemand_sites <- uptake_dyn %>%
 dplyr::filter(Ndemand > high_Ndemand_threshold) %>%
...
high_NO3_sup_sites <- uptake_dyn %>%
 dplyr::filter(NO3_supply > high_NO3_sup_threshold) %>%
 dplyr:: select(site, date, water_year, jday, Ndemand, NO3_supply_new, U_NO3_calc) %>%
...
high_NH4_sup_sites <- uptake_dyn %>%
 dplyr::filter(NH4_supply > high_NH4_sup_threshold) %>%
...
TS_high_N_demand<- ggplot(uptake_dyn%>%dplyr::filter(water_year<2024), aes(x = date, y = Ndemand, color = jday, shape = site)) +
  geom_point(size = 3, alpha = 0.95, position = position_dodge(width = 0.3)) +
  scale_color_viridis_c(option = "viridis", name = "Day of year") +
  geom_point(size = 2, alpha = 0.7) +
    scale_shape_manual(values = c(15, 0, 17, 2)) +
...
TS_high_NO3_sup <- ggplot(uptake_dyn%>%dplyr::filter(water_year<2024), aes(x = date, y = NO3_supply_new, color = jday, shape = site)) +
  geom_point(size = 3, alpha = 0.95, position = position_dodge(width = 0.3)) +
  scale_color_viridis_c(option = "viridis", name = "Day of year") +
  geom_point(size = 2, alpha = 0.7) +
    scale_shape_manual(values = c(15, 0, 17, 2)) +
...
TS_high_NH4_sup <- ggplot(uptake_dyn%>%dplyr::filter(water_year<2024), aes(x = date, y = NH4_supply_new, color = jday, shape = site)) +
  geom_point(size = 3, alpha = 0.95, position = position_dodge(width = 0.3)) +
  scale_color_viridis_c(option = "viridis", name = "Day of year") +
  geom_point(size = 2, alpha = 0.7) +
    scale_shape_manual(values = c(15, 0, 17, 2)) +
...
uptake_dyn_sum <- uptake_dyn2%>%
 ...
 dplyr::summarise(frac_NO3_uptake = mean(frac_NO3_uptake, na.rm=T),
...
reach_rent <- uptake_dyn %>%
 dplyr::filter(site %in% c("BWU", "BWL", "GBU", "GBL")) %>%
  group_by(catch, date) %>%
  summarize(
...
Rent_NO3_plt <- ggplot(reach_rent%>%dplyr::filter(water_year<2024), aes(x = date, y = NO3_retention, color = catch, shape = catch)) +
...
Rent_NH4_plt <- ggplot(reach_rent %>%dplyr::filter(water_year < 2024), 
...
reach_rent_sum <- reach_rent_sum %>%
 ...
 dplyr::summarise(NO3_retention = mean(NO3_retention, na.rm=T),
...
date_range <- reach_rent %>%
 dplyr::filter(water_year<2024) %>%
  dplyr::filter(!is.na(NH4_retention) | !is.na(NO3_retention)) %>%
  group_by(catch) %>%
  summarize(
...
summary_mean
summary_min <- BWL_DF %>%
   dplyr::summarise(across(everything(), min, na.rm = TRUE), .groups = "drop")
summary_min
summary_max <- BWL_DF %>%
...
BM_cor_plt <- ggplot(covariat_datq, aes(x =AFDM_ugcm2, y = GPP_mean)) + 
  ...
  theme_classic() + 
      # geom_smooth(data = covariat_datq%>%dplyr::filter(catch=="BW") , aes(color = "grey50"), method = "lm", se = FALSE, alpha = 0.5) + 
            scale_x_continuous(trans = "log", breaks = scales::log_breaks(base = 10),
                               labels = scales::label_number()) + 
      scale_y_continuous(trans = "log", 
                         breaks = scales::log_breaks(base = 10), 
...
N_P_ratio_plots_sw <- ggplot(Stoich_df %>%dplyr::filter(water_year<2024), aes(x = jday, y = N_P_ratio_sw, color = site)) +
...
N_P_ratio_plots <- ggplot(Stoich_df %>%dplyr::filter(water_year<2024), aes(y = GPP_mean, x = N_P_ratio_sw, color = site)) +
  geom_point(size = 2, alpha = 0.75, position = position_dodge(width = 0.3)) +
  scale_color_manual(values = site_colors) +
...
Stoich_table_sum <- Stoich_df%>%
 dplyr::filter(!is.na(N_P_ratio_sw))%>%
 dplyr::filter(N_P_ratio_sw>15.5) 
...
covariat_stats_summary <- covariat_stats %>%
  group_by(site, WY_lab) %>%
 dplyr::summarise(across(c(Ndemand, NO3_supply_new, NH4_supply_new), mean, na.rm = TRUE), .groups = "drop") %>%
...
valid_weeks <- covariat_dat_bp %>%
 dplyr::filter(site == "BWL", !is.na(GPP_mean)) %>%
  mutate(week = week(date)) %>%
  group_by(week, water_year) %>%
 dplyr::summarise(n_nonNA = n(), .groups = "drop") %>%
  group_by(week) %>%
 dplyr::summarise(n_years_with_data = n_distinct(water_year)) %>%
 dplyr::filter(n_years_with_data == n_years) %>%
  pull(week)
...
plot_BW_OM <- ggplot(
  covariat_dat_bp %>% 
   dplyr::filter(site == "BWL", water_year %in% 2022:2023),  # only show 2022–2023 data
  ...
  # geom_blank(data = covariat_dat_bp %>%dplyr::filter(water_year == 2021), aes(x = factor(2021), y = 10)) +
...
plot_BWU_OM <- ggplot(
  covariat_dat_bp %>% 
   dplyr::filter(site == "BWU", water_year %in% 2022:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = AFDM_mgg)) +
...
anova_n03_sup_bw <- aov(NO3_supply_new ~ as.factor(water_year), data = uptake_dyn2%>%dplyr::filter(site == "BWL", water_year %in% 2021:2023))
...
anova_n03_sup_bw <- aov(NO3_supply_new ~ as.factor(water_year), data = uptake_dyn2%>%dplyr::filter(site == "BWU", water_year %in% 2021:2023))
summary(anova_n03_sup_bw)
...
anova_n03_sup_gb <- aov(NO3_supply_new ~ as.factor(water_year), data = uptake_dyn2%>%dplyr::filter(site == "GBL", water_year %in% 2021:2023))
...
anova_n03_bw<- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>%dplyr::filter(site == "BWL", water_year %in% 2021:2023))
...
plot_Nsupply_BWL <- ggplot(
  uptake_dyn2 %>% 
   dplyr::filter(site == "BWL", water_year %in% 2021:2023),  # only show 2022–2023 data
...
anova_n <- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>%dplyr::filter(site == "BWU", water_year %in% 2021:2023))
...
plot_Nsupply_BWU <- ggplot(
  uptake_dyn2 %>% 
   dplyr::filter(site == "BWU", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NO3_supply_new)) +
...
### anova 
anova_n <- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>%dplyr::filter(site == "GBL", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
plot_Nsupply_GBL <- ggplot(
  uptake_dyn2 %>% 
   dplyr::filter(site == "GBL", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NO3_supply_new)) +
...
### anova 
anova_n <- aov(NO3_supply_new ~ as.factor(water_year), 
                        data = uptake_dyn2 %>%dplyr::filter(site == "GBU", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
plot_Nsupply_GBU <- ggplot(
  uptake_dyn2 %>% 
   dplyr::filter(site == "GBU", water_year %in% 2021:2023),  # only show 2022–2023 data
  aes(x = factor(water_year, levels = desired_years), y = NO3_supply_new)) +
...
date_datf_plt_sum <- covariat_datq %>%
 ...
 dplyr::filter(yday>1 & yday<285)%>%
 dplyr::filter(site=="BWL" & year ==2024) %>%
  group_by(year) %>%
 dplyr::summarise(T_q = max(Q_m, na.rm = TRUE),
...
anova_n <- aov(NO3_processedi ~ as.factor(water_year), 
                        data = reach_rent %>%dplyr::filter(catch == "BW", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
...
anova_n <- aov(NO3_processed ~ as.factor(water_year), 
                        data = reach_rent %>%dplyr::filter(catch == "GB", water_year %in% 2021:2023))
tukey_result <- TukeyHSD(anova_n)
