...
knitr::opts_chunk$set(
  ...
  warning = FALSE,
  message = FALSE,
  fig.path = "figures/",   # figures saved to a figures/ subfolder
  out.width = "100%"
)
...
covariat_datq <- readRDS("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/analysis/Mtn-stream-biogeochem-productivity/complied_data_JGR_MS_metab_26_06_rev.RDS")
```
...
#   scale_color_viridis_d(option = "viridis", name = "Water year") +
  theme_classic() +
          annotate("text", x = 7, y = -28, label = "Lower reach",size =4, fontface = "bold", hjust = 0.5) 
# 
...
#   scale_color_viridis_d(option = "viridis", name = "Water year") +
  theme_classic() +
            annotate("text", x = 7, y = -28, label = "Upper reach", size = 4, fontface = "bold", hjust = 0.5) 
...
GBU_KD_plot <- ggplot() +
  geom_point(data = GBU_DF_clean, aes(x = GPP_mean, y = ER_mean, color = as.factor(water_year)), alpha = 0.5, shape=2) +
    geom_contour(data = density_df, aes(x = x, y = y, z = z), color = "#4bcc94", bins = 5, size=1) +
  geom_abline(slope = -1, intercept = 0, linetype = "dashed", color = "black") +  # 1:1 line
...
GBU_KD_plot2 <- ggplot(covariat_datq%>%dplyr::filter(site=="GBU" & water_year<2025), aes(x = GPP_mean, y = ER_mean, color = as.factor(water_year))) +
  geom_density_2d(bins=10, size=1) + 
...
GBL_KD_plot2 <- ggplot(covariat_datq%>%dplyr::filter(site=="GBL" & water_year<2025), aes(x = GPP_mean, y = ER_mean, color = as.factor(water_year))) +
  geom_density_2d(bins=10, size=1) + 
  geom_point(alpha=0.5, shape=17)+
  geom_abline(slope = -1, intercept = 0, linetype = "dashed", color = "black") +  # 1:1 line
xlab(expression(GPP~(g~O[2]~m^-2~d^-1))) +
...
GBL_KD_plot <- GBL_KD_plot2 + 
  guides(color = guide_legend(title = "Water year", nrow = 2))

GBU_KD_plot <- GBU_KD_plot2 + 
  guides(color = guide_legend(title = "Water year", nrow = 2))
```
...
# ggsave("./Fig3_KD_CH1_grid_06_rev.png", plot = KD_coeff_grid, width = 6.25, height = 6.25, units = "in")
...
```{r, warning=F, size = 'small', echo=F, fig.width=9, fig.height=3}
library(lme4)

bw_dat <- covariat_datq %>% filter(catch == "BW")


gpp_mod_bw <- lmer(
  GPP_mean ~ scale(PAR_inc) +
 #   scale(wt) +
    scale(Q_m) +
    (1|site),
  data = bw_dat
...
```{r, warning=F, size = 'small', echo=F, fig.width=5, fig.height=3}

acf(residuals(gpp_mod_bw),
    main = "ACF of GLMM residuals")
```
...
bw_dat <- covariat_datq %>%
  filter(catch == "BW") %>%
  group_by(site, date) %>%
  summarize(
    GPP_mean = mean(GPP_mean, na.rm = TRUE),
    PAR_inc = mean(PAR_inc, na.rm = TRUE),
    Q_m = mean(Q_m, na.rm = TRUE),
...
bw_gpp_ar1 <- lme(
  GPP_mean ~ scale(PAR_inc) + scale(Q_m), #+ scale(wt),
  random = ~ 1 | site,
...
summary(bw_gpp_ar1)
acf(resid(bw_gpp_ar1, type = "normalized"),
    main = "ACF of AR(1) model residuals")

 r.squaredLR(bw_gpp_ar1)
```
...
bw_dat <- covariat_datq %>%
  ...
  group_by(site, date) %>%
  summarize(
    ER_mean = mean(ER_mean, na.rm = TRUE),
    Q_m = mean(Q_m, na.rm = TRUE),
    wt = mean(wt, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
summary(bw_er_ar1)
acf(resid(bw_er_ar1, type = "normalized"),
    main = "ACF of AR(1) model residuals")

r.squaredLR(bw_er_ar1)
...
gb_dat <- covariat_datq %>% filter(catch == "GB")
...
summary(gb_gpp_ar1)
acf(resid(gb_gpp_ar1, type = "normalized"),
    main = "ACF of AR(1) model residuals")
...
summary(gb_er_ar1)
acf(resid(gb_er_ar1, type = "normalized"),
    main = "ACF of AR(1) model residuals")

 r.squaredLR(gb_er_ar1)
```
...
```{r, warning=F, size = 'small', echo=F, include=FALSE}
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/Fig2_baseflow_combined_rev_06.png", plot = baseflow_grid, width =5.5, height = 6.5, units = "in")
...
# ggsave("./Nsupply_grid_CH1_SFS_ppt_21_rev_06.png", plot = n_s_grid, width = 8.5, height = 7.75, units = "in")
...
```{r, warning=F, size = 'small', echo=F, include=F, fig.width=9, fig.height=4}
# ggsave("./Fig4_n_dyn_rev_06.png", plot = Ndyn_grid, width = 10.75, height = 6.75, units = "in")
...
```{r, warning=F, size = 'small', echo=F, include=FALSE}
# ggsave("./Figure6_annualNtrend_rev_06.png", plot = Ndyn_annual_grid, width = 8, height = 6.5, units = "in")
...
GBL_plot1 <- ggplot(date_datf_plt_gb, aes(x = yday, y = ER_cumsum, color = factor(year))) +
  ...
  scale_color_manual(values=c("#440154", "#3B528B", "#4bcc94")) +
 #   scale_color_viridis_d(option = "viridis",  name = "Year") +
  theme_classic() + xlim(91, 335) +ylim(0,200)
#xlim(115, 332)
...
```{r, warning=F, size = 'small', echo=F, include=FALSE}
# ggsave("./Fig_cumsum_GPP_rev_06.png", plot = Csum_grid_T, width = 5.5, height = 8.25, units = "in")
