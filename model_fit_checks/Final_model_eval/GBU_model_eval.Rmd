---
title: 'Streammetabolizer output for GBU'
author: "Kelly Loria"
date: "2025-02-04"
output:
   html_document:
...
met.full <- read_csv("./GBU_Full_2025-02-04_daily.csv") %>%
  mutate(year = year(date))%>%
  mutate(jday = yday(date))%>%
  mutate(mon = month(date))%>%
   mutate(lab = "Full")%>%
...
fits.full <- read_csv("./GBU_Full_2025-02-04_errors.csv_mod_and_obs_DO.csv") %>%
  mutate(week = week(solar.time))%>%
  mutate(jday = yday(solar.time)) %>%
     mutate(lab = "Full")%>%
  mutate(
...
# high flow days
dat2 <- input_dat%>% filter(discharge>1.16) %>%
  mutate(date =as.Date(solar.time))
...
excluded_summary <- tibble(
  `Number of Days` = c(total_days, num_na_clean, high_flow_day,
                       ...
                       clean_gpp_day,
                       clean_ER_day), 
  `Explaination` = c("Total days of DO observations", "Total days removed", "Days removed for high flow",
