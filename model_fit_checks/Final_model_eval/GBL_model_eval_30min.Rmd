---
title: 'Streammetabolizer output for GBL- 30 minute obs'
author: "Kelly Loria"
date: "2025-03-27"
output:
  github_document:
...
met.full <- read_csv("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/analysis/GBL_metab_revision/GBL_Bayes_linearK600_measured_prior_20260607_fit.csv") %>%
  mutate(year = year(date))%>%
  mutate(jday = yday(date))%>%
  mutate(mon = month(date))%>%
   mutate(lab = "Full")%>%
...
# fits.full <- read_csv("./GBL_Full_30_2025-03-27_errors.csv_mod_and_obs_DO.csv") %>%
#   mutate(week = week(solar.time))%>%
#   mutate(jday = yday(solar.time)) %>%
#      mutate(lab = "Full")%>%
#   mutate(
...
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/GBL_model_fits.png", plot = final_plot, width = 8, height = 6, units = "in")
