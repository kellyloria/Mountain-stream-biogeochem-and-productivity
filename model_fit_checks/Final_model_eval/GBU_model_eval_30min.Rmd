...
```{r, warning=FALSE, size='small', echo=FALSE, fig.align="center", fig.width=8, fig.height=5}
...
met.full <- read_csv("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/analysis/GBL_metab_revision/GBU_Bayes_linearK600_measured_prior_20260620_fit_daily.csv") %>%
  mutate(
    date = as.Date(date),
    year = year(date),
    jday = yday(date),
    mon = month(date),
    lab = "Full",
    NEP_daily_mean = GPP_daily_mean + ER_daily_mean
  )
...
gpp_unit <- expression(
  paste("GPP (g O"[2], " ", m^{-2}, " ", d^{-1}, ")")
)

er_unit <- expression(
  paste("|ER| (g O"[2], " ", m^{-2}, " ", d^{-1}, ")")
)

k600_unit <- expression(
  paste(K[600], " (m ", d^{-1}, ")")
)
...
# ggsave("/Users/kellyloria/Documents/Publications/CH1\ biogeochem\ linkages/Figs/GBU_model_fits.png", plot = final_plot, width = 8, height = 6, units = "in")
