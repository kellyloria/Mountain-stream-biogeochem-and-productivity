---
title: 'Streammetabolizer output for BWL'
author: "Kelly Loria"
date: "2025-02-06"
output:
   html_document:
...
met.full <- read_csv("./Full/BWL_Full_2025-02-06_daily.csv") %>%
  mutate(year = year(date))%>%
  mutate(jday = yday(date))%>%
  mutate(mon = month(date))%>%
   mutate(lab = "Full")%>%
...
fits.full <- read_csv("./Full/BWL_Full_2025-02-06_errors.csv_mod_and_obs_DO.csv") %>%
  mutate(week = week(solar.time))%>%
  mutate(jday = yday(solar.time)) %>%
     mutate(lab = "Full")%>%
  mutate(
