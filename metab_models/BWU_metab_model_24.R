...
site <- "BWU"
dat <- readRDS("./Stream_Metab_24/Core_sites/offset_DO_dat/light/25_BWU_DO_flag_sat_light_v2.rds") %>%
  filter(solar.time > as.POSIXct("2021-06-30 09:45:00"))
...
output_path <- "./Stream_Metab_24/model_output/BWU"
...
# 
ggsave(plot = DOplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/DO_preds_BWU_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)

metabplot<- plot_metab_preds(predict_metab(dat_metab_BW))
# 
ggsave(plot = metabplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/metabplot_BWU_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)
...
ggsave(plot = binplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/binplot_BWU_bayesmodel_measuredK600_no_filter.png",sep=""),width=5,height=4,dpi=300)
...
# saveRDS(dat_metab_BW, file = "./Stream_Metab_24/24_metabolism_output/Full_out/BWU_Summer_K600_measured_prior_241208.rds")
...
# plot for k600
BWU <- read.csv("./Stream_Metab_24/24_metabolism_output/BWU_K600_measured_prior_noGPP_prior_no_filter_NLDAS_daily241213.csv")

BWU$date <- as.Date(BWU$date, origin="2021-01-01")
BWU$site <- "BWU"
BWU$shore <- "west"

Odd_plot <- ggplot(BWU, aes(x = K600_daily_mean, color = shore, fill = shore)) +
  ...
  geom_vline(data = BWU, aes(xintercept = mean(na.omit(K600_daily_mean))), linetype = "dashed") 

Gplot_sp <- ggplot(BWU, aes(x = K600_daily_mean, y = (ER_mean*-1))) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  geom_smooth(method ="lm", se=F)+  facet_grid(.~site)+
  theme_bw()


GB_lm <- lm(K600_daily_mean~(ER_mean*-1), data=BWU)
summary(GB_lm)

# Extract R-squared value
r_gsquared <- summary(GB_lm)$r.squared
GB_plot <- Gplot_sp + 
  geom_text(aes(x = mean((BWU$K600_daily_mean), na.rm=T), y = max((BWU$ER_mean*-1), na.rm=T), 
...
#
ggsave(plot = k_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/ER_K600plot_BWU_K600_measured_prior_no_filter_daily241213.png",sep=""),width=6,height=3,dpi=300)
...
Gplot_DOsat <- ggplot(BWU2, aes(y = K600_daily_mean, x = DO.sat)) +  
...
Gplot_temp <- ggplot(BWU2, aes(y = K600_daily_mean, x = temp.water)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+ 
  facet_grid(.~site)+
  theme_bw()

Gplot_Q <- ggplot(BWU2, aes(y = K600_daily_mean, x = discharge)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+  
  facet_grid(.~site)+
  theme_bw()
...
GB_lmq <- lm(K600_daily_mean~discharge, data=BWU2)
summary(GB_lmq)

# Extract R-squared value
r_gsquaredq <- summary(GB_lmq)$r.squared
Gplot_Q2 <- Gplot_Q + 
  geom_text(aes(y = mean((BWU2$K600_daily_mean), na.rm=T), x = max((BWU2$discharge), na.rm=T), 
                label = paste("R2=", round(r_gsquaredq, 3))),
            hjust = 1, vjust = 0, size = 3, col = "blue",
            parse = T, check_overlap = T, na.rm = T)

Gplot_z <- ggplot(BWU2, aes(y = K600_daily_mean, x = depth)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+  
  facet_grid(.~site)+
  theme_bw()

GB_lmz <- lm(K600_daily_mean~depth, data=BWU2)
summary(GB_lmz)
r_gsquaredz <- summary(GB_lmz)$r.squared

Gplot_z2 <- Gplot_z + 
  geom_text(aes(y = mean((BWU2$K600_daily_mean), na.rm=T), x = max((BWU2$depth), na.rm=T), 
...
ggsave(plot = k_e_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/BWU_K600_conditions_plot_measured_noGPP_prior_no_filter_241213.png",sep=""),width=4,height=12,dpi=300)
