...
site <- "GBU"
dat <- readRDS("./Stream_Metab_24/Core_sites/offset_DO_dat/light/25_GBU_DO_flag_sat_light_v2.rds")%>%
  mutate(date= as.Date.character(solar.time))

dat <- dat %>%
  filter(fouling_flag<1 & do.obs > 3.75)
summary(dat)
names(dat)

dat <- dat %>%
  filter(!(solar.time > as.POSIXct("2021-10-20 08:00:00") & solar.time < as.POSIXct("2021-10-20 09:00:00"))) %>%
  filter(!(solar.time > as.POSIXct("2023-03-06 20:30:00") & solar.time < as.POSIXct("2023-03-09 12:00:00"))) %>%
  filter(!(solar.time > as.POSIXct("2023-05-06 09:30:00") & solar.time < as.POSIXct("2023-05-09 09:00:00"))) %>%
  filter(!(solar.time > as.POSIXct("2024-10-08 10:30:00") & solar.time < as.POSIXct("2024-10-10 12:00:00")))




# List of days to remove
negative_days2 <- c( "2021-05-15", "2021-05-16", "2021-05-17",
                     ...
                     "2021-09-23",  "2021-09-27", "2021-12-26", "2022-02-26",
                     "2022-02-27",
                     "2022-11-19",
                     "2022-12-12",
                      "2022-12-30", "2023-05-10",
...
dat <- dat[!dat$date %in% negative_days2, ]
...
# Smaller chunks of time for preliminary models 
...
lowQ2_test <- subset(dat, solar.time > "2022-09-02 00:00:00" & solar.time < "2022-09-04 02:00:00")

Summer_test <- subset(dat, solar.time > "2021-08-06 00:00:00" & solar.time < "2021-08-09 00:00:00")

## Gas exchange 1:
Summer_test2 <- subset(dat, solar.time > "2021-08-06 00:00:00" & solar.time < "2021-08-09 00:00:00")

# Make of all combined plot where x axis flow y axis k600 
...
## Set bayes specs
bayes_name_new <- mm_name(type = 'bayes', pool_K600 = "normal",
...
# separate bin for each magnitude change 
bayes_specs_new$K600_lnQ_nodes_centers <- seq(c(logQ_range[1]+1.25), c(logQ_range[2]-1.25), length.out = 6)
...
bayes_specs_new$K600_daily_sigma_sigma <- 0.05 # bob likes this number 
...
site <- "GBU"
rundate <- Sys.Date()
label <- "hr_tvl_flagged_flow_excluded_badday"


## PAUSE ! check out path 
output_path <- "./Stream_Metab_24/model_output/GBU_Hr_tvlbp"
...
# 
ggsave(plot = DOplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/DO_preds_GBU_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)

metabplot<- plot_metab_preds(predict_metab(dat_metab_GB))
# 
ggsave(plot = metabplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/metabplot_GBU_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)
...
ggsave(plot = binplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/binplot_GBU_bayesmodel_measuredK600_no_filter.png",sep=""),width=5,height=4,dpi=300)

# ggsave(plot = binplot, filename = paste("./figures/binplot_GBU_bayesmodel_GBU2.png",sep=""),width=5,height=4,dpi=300)
...
# saveRDS(dat_metab_GB, file = "./Stream_Metab_24/24_metabolism_output/Full_out/GBU_Summer_K600_measured_prior_241208.rds")
...
# plot for k600
GBU <- read.csv("./Stream_Metab_24/24_metabolism_output/GBU_K600_measured_prior_noGPP_prior_no_filter_NLDAS_daily241213.csv")
...
#
ggsave(plot = k_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/ER_K600plot_GBU_K600_measured_prior_no_filter_daily241213.png",sep=""),width=6,height=3,dpi=300)
...
ggsave(plot = k_e_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/GBU_K600_conditions_plot_measured_noGPP_prior_no_filter_241213.png",sep=""),width=4,height=12,dpi=300)
