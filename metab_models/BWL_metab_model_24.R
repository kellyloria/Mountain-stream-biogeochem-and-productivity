...
site <- "BWL"
dat <- readRDS("./Stream_Metab_24/Core_sites/offset_DO_dat/light/25_BWL_DO_flag_sat_light_v2.rds")
...
lowQ2_test <- subset(dat, solar.time > "2022-09-04 00:00:00" & solar.time < "2022-09-05 20:00:00")

Summer_test <- subset(dat, solar.time > "2021-07-05 00:00:00" & solar.time < "2021-09-30 00:00:00")
...
k600 = c((181.13 * dat$discharge) + 8.8121)
hist(log(k600))
summary(log(k600))
...
bayes_specs_new$K600_lnQ_nodes_centers <- seq(c(logQ_range[1]+0.5), c(logQ_range[2]-0.5), length.out = 6)
bayes_specs_new$K600_lnQ_nodes_meanlog <- c(4.30, 
                                            3.79,
                                            3.27, 
                                            2.78, 
                                            2.69,
...
site <- "BWL"
rundate <- Sys.Date()
label <- "TVL_flagged_flow_excluded"


## check out path 
output_path <- "./Stream_Metab_24/model_output/BWL_tvlbp"
...
save(dat_fit_BW, dat_metab_BW, file = save_path)
...
# Run the function
writefiles(dat_fit_BW, dat_metab_BW, path = output_path)
...
library(ggpubr)
##################
mod.fits <- read.csv(paste0(path, site, "_", label, "_", rundate, "_", "mod_and_obs_DO.csv")) %>%
...
# 
ggsave(plot = DOplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/DO_preds_BWL_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)

metabplot<- plot_metab_preds(predict_metab(dat_metab_BW))
# 
ggsave(plot = metabplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/metabplot_BWL_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)
...
ggsave(plot = binplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/binplot_BWL_bayesmodel_measuredK600_no_filter.png",sep=""),width=5,height=4,dpi=300)

# ggsave(plot = binplot, filename = paste("./2023_StreamMetab/figures/binplot_BWL_bayesmodel_BWL2.png",sep=""),width=5,height=4,dpi=300)
...
# saveRDS(dat_metab_BW, file = "./Stream_Metab_24/24_metabolism_output/Full_out/BWL_Summer_K600_measured_prior_241208.rds")
...
# plot for k600
BWL <- read.csv("./Stream_Metab_24/24_metabolism_output/BWL_K600_measured_prior_noGPP_prior_no_filter_NLDAS_daily241213.csv")
...
Gplot_sp <- ggplot(BWL, aes(x = K600_daily_mean, y = (ER_mean*-1))) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  geom_smooth(method ="lm", se=F)+  facet_grid(.~site)+
  theme_bw()


GB_lm <- lm(K600_daily_mean~(ER_mean*-1), data=BWL)
summary(GB_lm)

# Extract R-squared value
r_gsquared <- summary(GB_lm)$r.squared
GB_plot <- Gplot_sp + 
  geom_text(aes(x = mean((BWL$K600_daily_mean), na.rm=T), y = max((BWL$ER_mean*-1), na.rm=T), 
...
#
ggsave(plot = k_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/ER_K600plot_BWL_K600_measured_prior_no_filter_daily241213.png",sep=""),width=6,height=3,dpi=300)
...
Gplot_DOsat <- ggplot(BWL2, aes(y = K600_daily_mean, x = DO.sat)) +  
...
Gplot_temp <- ggplot(BWL2, aes(y = K600_daily_mean, x = temp.water)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+ 
  facet_grid(.~site)+
  theme_bw()

Gplot_Q <- ggplot(BWL2, aes(y = K600_daily_mean, x = discharge)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+  
  facet_grid(.~site)+
  theme_bw()
...
GB_lmq <- lm(K600_daily_mean~discharge, data=BWL2)
summary(GB_lmq)

# Extract R-squared value
r_gsquaredq <- summary(GB_lmq)$r.squared
Gplot_Q2 <- Gplot_Q + 
  geom_text(aes(y = mean((BWL2$K600_daily_mean), na.rm=T), x = max((BWL2$discharge), na.rm=T), 
                label = paste("R2=", round(r_gsquaredq, 3))),
            hjust = 1, vjust = 0, size = 3, col = "blue",
            parse = T, check_overlap = T, na.rm = T)

Gplot_z <- ggplot(BWL2, aes(y = K600_daily_mean, x = depth)) +  
  geom_point(shape= 17, col = alpha(c("#3283a8"),0.75)) +
  #geom_smooth(method ="lm", se=F)+  
  facet_grid(.~site)+
  theme_bw()

GB_lmz <- lm(K600_daily_mean~depth, data=BWL2)
summary(GB_lmz)
r_gsquaredz <- summary(GB_lmz)$r.squared

Gplot_z2 <- Gplot_z + 
  geom_text(aes(y = mean((BWL2$K600_daily_mean), na.rm=T), x = max((BWL2$depth), na.rm=T), 
...
ggsave(plot = k_e_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/BWL_K600_conditions_plot_measured_noGPP_prior_no_filter_241213.png",sep=""),width=4,height=12,dpi=300)
