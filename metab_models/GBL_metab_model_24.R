...
site <- "GBL"
dat <- readRDS("./25_GBL_DO_flag_sat_light_v2.rds")
...
dat1 <- dat %>%
  filter(!(solar.time > as.POSIXct("2022-04-07 02:00:00") & solar.time < as.POSIXct("2022-04-07 04:00:00"))) %>%
  filter(!(solar.time > as.POSIXct("2022-11-04 00:00:00") & solar.time < as.POSIXct("2022-11-04 12:00:00"))) %>%
  filter(!(solar.time > as.POSIXct("2023-03-27 00:00:00") & solar.time < as.POSIXct("2023-03-27 12:00:00")))

dat1 <- dat1%>%
  mutate(date= as.Date(solar.time))


# List of days to remove
negative_days2 <- c("2021-06-29", "2021-06-30", "2021-07-01", "2021-07-02", 
                    "2021-07-05", "2021-07-06", "2021-07-07", "2021-07-23",
                    "2021-07-24", "2021-07-25", "2021-07-26", "2021-07-27", 
                    "2021-07-28", "2021-07-29","2021-08-18", "2021-08-19", 
                    ...
                    "2021-09-08", "2021-09-09", "2021-09-13", "2021-09-18",
                    "2021-09-19", "2021-09-23", 
                    "2022-07-21", "2022-07-22", "2022-07-23", "2022-07-24", 
                    "2022-07-25", "2022-07-27", "2022-07-28", "2022-08-07",
                    "2022-08-08", "2022-08-09",  "2022-08-10", "2022-08-17",
                    "2022-08-18" ) 


negative_days3 <- c("2021-05-14", "2021-05-16", "2021-05-17",
                    "2021-06-29", "2021-06-30", "2021-07-01", "2021-07-02", 
                    "2021-07-05", "2021-07-06", "2021-07-07","2021-07-21", 
                    ...
                    "2021-09-04", "2021-09-05", "2021-09-06", "2021-09-07", 
                    "2021-09-08", "2021-09-09", "2021-09-13", "2021-09-14", 
                    "2021-09-15", "2021-09-16", "2021-09-17",
                    "2021-09-18",
                    "2021-09-19", "2021-09-20", "2021-09-21", "2021-09-22",
                    "2021-09-23", "2021-09-26", "2021-09-27",
                    "2021-10-05", 
                    "2021-10-24", "2021-12-26", "2022-01-28",
                    "2022-01-29", "2022-02-26", "2022-02-27",
                    "2022-07-21", "2022-07-22", "2022-07-23", "2022-07-24", 
                    "2022-07-25", "2022-07-27", "2022-07-28", "2022-07-29" ,
                    "2022-08-03", "2022-08-04",
                    "2022-08-07",
                    "2022-08-08", "2022-08-09",  "2022-08-10", 
                    "2022-08-11", "2022-08-12",  "2022-08-13",  "2022-08-14",
                    "2022-08-15",
                    "2022-08-17", "2022-08-18", "2022-09-25", 
                    "2022-09-30", "2022-11-18", "2022-12-12", 
                    "2022-12-13", "2023-01-30",
                    "2023-03-02", "2023-05-13", "2023-05-14", "2023-06-17",
                    "2023-08-18",
                    "2023-08-28", "2023-09-13", "2024-01-07", "2024-09-22", 
                    "2024-09-23") 

dat <- dat1[!dat1$date %in% negative_days3, ]


datt <- dat %>%
  # Round datetime to the nearest hour
  mutate(
    solar.time = as.POSIXct(round(as.numeric(solar.time) / 3600) * 3600, origin = "1970-01-01", tz = "America/Los_Angeles")
  ) %>%
  # Group by rounded datetime and calculate means
  group_by(solar.time) %>%
  summarize(
    DO.obs = mean(do.obs, na.rm = TRUE),
    DO.sat = mean(DO.sat, na.rm = TRUE),
    temp.water = mean(wtr, na.rm = TRUE),
    depth = mean(depth, na.rm = TRUE),
    discharge = mean(dischargeCMS, na.rm = TRUE),
    light = mean(light_in, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Add a date-only column for convenience
  mutate(
    date_only = as.Date(solar.time)
  ) %>%
  # Ensure regular time intervals (hourly) for the entire time range
  complete(
    solar.time = seq(min(solar.time), max(solar.time), by = "hour")
  ) %>%
  # Fill missing values using nearest neighbor interpolation
  mutate(
    DO.obs = zoo::na.locf(DO.obs, na.rm = FALSE, fromLast = TRUE, maxgap = 3),
    DO.sat = zoo::na.locf(DO.sat, na.rm = FALSE, fromLast = TRUE, maxgap = 3),
    temp.water = zoo::na.locf(temp.water, na.rm = FALSE, fromLast = TRUE, maxgap = 3),
    depth = zoo::na.locf(depth, na.rm = FALSE, fromLast = TRUE, maxgap = 3),
    discharge = zoo::na.locf(discharge, na.rm = FALSE, fromLast = TRUE, maxgap = 3),
    light = zoo::na.locf(light, na.rm = FALSE, fromLast = TRUE, maxgap = 3)
  )


#datt$solar.time <- calc_solar_time(datt$solar.time, -120.164335)
...
# Subsets for smaller times of flow:
...
lowQ2_test <- subset(dat, solar.time > "2023-08-07 00:00:00" & solar.time < "2023-08-09 20:00:00")
...
bayes_specs_new$K600_lnQ_nodes_centers <- seq(c(logQ_range[1]+1.5), c(logQ_range[2]-1.5), length.out = 6)
...
site <- "GBL"
rundate <- Sys.Date()
label <- "Hr_tvl_bad_days_trimER"


## PAUSE ! check out path 
output_path <- "/Stream_Metab_24/model_output/GBL_Hr_tvl_bad_day_ER"
...
writefiles <- function(data, data2, path) {
  for (i in seq_along(data)) {
    filename <- paste0(site, "_", label, "_", rundate, "_", names(data)[i], ".csv")
    write.csv(data[[i]], filename, row.names = FALSE)
  }
  
  write.csv(unlist(get_specs(data2)), paste0(site, "_", label, "_", rundate, "_", "specs.csv"), row.names = FALSE)
  write.csv(get_data_daily(data2), paste0(site, "_", label, "_", rundate, "_", "datadaily.csv"), row.names = FALSE)
  write.csv(get_data(data2), paste0(site, "_", label, "_", rundate, "_", "mod_and_obs_DO.csv"), row.names = FALSE)
}
...
# 
ggsave(plot = DOplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/DO_preds_GBL_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)

metabplot<- plot_metab_preds(predict_metab(dat_metab_GB))
# 
ggsave(plot = metabplot, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_figure/metabplot_GBL_bayesmodel_measuredK600_no_filter.png",sep=""),width=10,height=4,dpi=300)
...
ggsave(plot = binplot, filename = paste("R:/Users/kloria/Documents/Stream_Metab_24/24_metabolism_output/Full_figure/binplot_GBL_bayesmodel_measuredK600_no_filter.png",sep=""),width=5,height=4,dpi=300)

# ggsave(plot = binplot, filename = paste("./binplot_GBL_bayesmodel_GBL2.png",sep=""),width=5,height=4,dpi=300)
...
## Save info:
writefiles <- function(data, data2, path = "./Stream_Metab_24/24_metabolism_output/") {
  for (i in seq_along(data)) {
...
# saveRDS(dat_metab_GB, file = "./Stream_Metab_24/24_metabolism_output/Full_out/GBL_Summer_K600_measured_prior_241208.rds")
...
# plot for k600
GBL <- read.csv("./Stream_Metab_24/24_metabolism_output/GBL_K600_measured_prior_noGPP_prior_no_filter_NLDAS_daily241213.csv")
...
#
ggsave(plot = k_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/ER_K600plot_GBL_K600_measured_prior_no_filter_daily241213.png",sep=""),width=6,height=3,dpi=300)
...
ggsave(plot = k_e_grid, filename = paste("./Stream_Metab_24/24_metabolism_output/Full_out/GBL_K600_conditions_plot_measured_noGPP_prior_no_filter_241213.png",sep=""),width=4,height=12,dpi=300)
